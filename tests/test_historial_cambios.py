...
funcion ni tocar Postgres para probarlo) -- {(dni, campo_lower): [(fecha_desde, fecha_hasta, valor, dia_semana, canal), ...]}.

`canal` (Davor, 2026-09-22, soporte de 2 turnos/día) es el 5to elemento de
cada tupla -- None significa "aplica a todos los turnos" (el 99% de las
filas, incluidas TODAS las anteriores a esta fecha)."""
...
def test_override_vigente_dentro_del_rango_de_fechas():
    idx = {("123", "zona"): [(dt.date(2026, 8, 1), dt.date(2026, 8, 31), "Zona Nueva", pd.NA, None)]}
...
def test_override_sin_fecha_hasta_sigue_vigente_indefinidamente():
    idx = {("123", "zona"): [(dt.date(2026, 8, 1), pd.NA, "Zona Nueva", pd.NA, None)]}
...
def test_override_con_dia_de_la_semana_solo_aplica_ese_dia():
    ...
    idx = {("123", "refrigerio"): [(dt.date(2026, 8, 1), None, "Sin refrigerio", 1, None)]}
...
def test_dia_semana_no_reconocido_nunca_esta_vigente():
    ...
    idx = {("123", "zona"): [(dt.date(2026, 8, 1), None, "Zona Nueva", -1, None)]}
...
def test_toma_el_mas_reciente_cuando_hay_varios_vigentes():
    idx = {("123", "zona"): [
        (dt.date(2026, 1, 1), None, "Zona Vieja", pd.NA, None),
        (dt.date(2026, 7, 1), None, "Zona Reciente", pd.NA, None),
    ]}
...
def test_hora_valida_hh_mm_se_normaliza_con_segundos():
    idx = {("123", "hora entrada programada"): [(dt.date(2026, 8, 1), None, "09:15", pd.NA, None)]}
...
def test_hora_invalida_cae_al_default_en_vez_de_tumbar_el_motor(capsys):
    ...
    idx = {("123", "hora salida programada"): [(dt.date(2026, 8, 1), None, "no es una hora", pd.NA, None)]}
...
def test_fecha_none_devuelve_el_default_sin_reventar():
    idx = {("123", "zona"): [(dt.date(2026, 8, 1), None, "Zona Nueva", pd.NA, None)]}
...
# -- Canal por turno (Davor, 2026-09-22, soporte de 2 turnos/día) --

def test_override_generico_sin_canal_aplica_a_cualquier_turno():
    idx = {("73897404", "hora entrada programada"): [(dt.date(2026, 8, 1), None, "07:15", pd.NA, None)]}
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "07:00:00", canal="Farmacia") == "07:15:00"
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "13:00:00", canal="Autoservicio") == "07:15:00"


def test_override_con_canal_solo_aplica_a_ese_turno():
    idx = {("73897404", "hora entrada programada"): [(dt.date(2026, 8, 1), None, "07:15", pd.NA, "Farmacia")]}
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "07:00:00", canal="Farmacia") == "07:15:00"
    # El turno de Autoservicio no debe verse afectado por un override que
    # es específicamente de Farmacia.
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "13:00:00", canal="Autoservicio") == "13:00:00"


def test_override_con_canal_gana_sobre_uno_generico_vigente_la_misma_fecha():
    idx = {("73897404", "hora entrada programada"): [
        (dt.date(2026, 8, 1), None, "07:30", pd.NA, None),
        (dt.date(2026, 8, 1), None, "07:15", pd.NA, "Farmacia"),
    ]}
    # El turno de Farmacia ve el override específico, no el genérico.
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "07:00:00", canal="Farmacia") == "07:15:00"
    # El turno de Autoservicio (sin override propio) sigue viendo el genérico.
    assert valor_efectivo(idx, "73897404", "Hora entrada programada", dt.date(2026, 8, 10), "13:00:00", canal="Autoservicio") == "07:30:00"
